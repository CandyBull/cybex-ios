//
//  UIColor+Additions.swift
//  Cybex IOS
//
//  Generated on Zeplin. (2018/8/31).
//  Copyright (c) 2018 __MyCompanyName__. All rights reserved.
//

import UIKit

extension UIColor {

  @nonobjc class var peach: UIColor {
    return UIColor(red: 1.0, green: 196.0 / 255.0, blue: 120.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var maincolor: UIColor {
    return UIColor(red: 1.0, green: 145.0 / 255.0, blue: 67.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var white: UIColor {
    return UIColor(white: 1.0, alpha: 1.0)
  }

    @nonobjc class var lightPrimary: UIColor {
        return UIColor(red: 164.0 / 255.0, green: 244.0 / 255.0, blue: 236.0 / 255.0, alpha: 1.0)
    }

    @nonobjc class var primaryGreen: UIColor {
        return UIColor(red: 2.0 / 255.0, green: 168.0 / 255.0, blue: 108.0 / 255.0, alpha: 1.0)
    }

    @nonobjc class var primaryRed: UIColor {
        return UIColor(red: 229.0 / 255.0, green: 81.0 / 255.0, blue: 81.0 / 255.0, alpha: 1.0)
    }

  @nonobjc class var primary: UIColor {
    return UIColor(red: 46.0 / 255.0, green: 216.0 / 255.0, blue: 216.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var steel50: UIColor {
    return UIColor(red: 120.0 / 255.0, green: 129.0 / 255.0, blue: 154.0 / 255.0, alpha: 0.5)
  }

  @nonobjc class var steel: UIColor {
    return UIColor(red: 120.0 / 255.0, green: 129.0 / 255.0, blue: 154.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var darkTwo: UIColor {
    return UIColor(red: 27.0 / 255.0, green: 34.0 / 255.0, blue: 48.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var paleGrey: UIColor {
    return UIColor(red: 247.0 / 255.0, green: 248.0 / 255.0, blue: 250.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var darkTwo50: UIColor {
    return UIColor(red: 27.0 / 255.0, green: 34.0 / 255.0, blue: 48.0 / 255.0, alpha: 0.5)
  }

  @nonobjc class var white80: UIColor {
    return UIColor(white: 1.0, alpha: 0.8)
  }

  @nonobjc class var reddish: UIColor {
    return UIColor(red: 190.0 / 255.0, green: 70.0 / 255.0, blue: 52.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var coolGrey: UIColor {
    return UIColor(red: 141.0 / 255.0, green: 144.0 / 255.0, blue: 151.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var turtleGreen: UIColor {
    return UIColor(red: 109.0 / 255.0, green: 187.0 / 255.0, blue: 73.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var darkTwo80: UIColor {
    return UIColor(red: 27.0 / 255.0, green: 34.0 / 255.0, blue: 48.0 / 255.0, alpha: 0.8)
  }

  @nonobjc class var steel80: UIColor {
    return UIColor(red: 120.0 / 255.0, green: 129.0 / 255.0, blue: 154.0 / 255.0, alpha: 0.8)
  }

  @nonobjc class var steel20: UIColor {
    return UIColor(red: 120.0 / 255.0, green: 129.0 / 255.0, blue: 154.0 / 255.0, alpha: 0.2)
  }

  @nonobjc class var black10: UIColor {
    return UIColor(white: 0.0, alpha: 0.1)
  }

  @nonobjc class var black4: UIColor {
    return UIColor(white: 0.0, alpha: 0.04)
  }

  @nonobjc class var white4: UIColor {
    return UIColor(white: 1.0, alpha: 0.04)
  }

  @nonobjc class var blueyGrey: UIColor {
    return UIColor(red: 183.0 / 255.0, green: 188.0 / 255.0, blue: 201.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var darkThree: UIColor {
    return UIColor(red: 22.0 / 255.0, green: 28.0 / 255.0, blue: 40.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var paleGreyTwo: UIColor {
    return UIColor(red: 253.0 / 255.0, green: 253.0 / 255.0, blue: 254.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var steel11: UIColor {
    return UIColor(red: 120.0 / 255.0, green: 129.0 / 255.0, blue: 154.0 / 255.0, alpha: 0.11)
  }

  @nonobjc class var dark20: UIColor {
    return UIColor(red: 23.0 / 255.0, green: 29.0 / 255.0, blue: 42.0 / 255.0, alpha: 0.2)
  }

  @nonobjc class var white98: UIColor {
    return UIColor(white: 1.0, alpha: 0.98)
  }

  @nonobjc class var steel10: UIColor {
    return UIColor(red: 120.0 / 255.0, green: 129.0 / 255.0, blue: 154.0 / 255.0, alpha: 0.1)
  }

  @nonobjc class var steel30: UIColor {
    return UIColor(red: 120.0 / 255.0, green: 129.0 / 255.0, blue: 154.0 / 255.0, alpha: 0.3)
  }

  @nonobjc class var darkFour: UIColor {
    return UIColor(red: 33.0 / 255.0, green: 41.0 / 255.0, blue: 57.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var paleOliveGreen: UIColor {
    return UIColor(red: 169.0 / 255.0, green: 224.0 / 255.0, blue: 110.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var apple: UIColor {
    return UIColor(red: 141.0 / 255.0, green: 200.0 / 255.0, blue: 79.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var pastelRed: UIColor {
    return UIColor(red: 217.0 / 255.0, green: 98.0 / 255.0, blue: 80.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var reddish15: UIColor {
    return UIColor(red: 190.0 / 255.0, green: 70.0 / 255.0, blue: 52.0 / 255.0, alpha: 0.15)
  }

  @nonobjc class var turtleGreen15: UIColor {
    return UIColor(red: 109.0 / 255.0, green: 187.0 / 255.0, blue: 73.0 / 255.0, alpha: 0.15)
  }

  @nonobjc class var black: UIColor {
    return UIColor(white: 0.0, alpha: 1.0)
  }

  @nonobjc class var fadedRed: UIColor {
    return UIColor(red: 215.0 / 255.0, green: 51.0 / 255.0, blue: 61.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var paleGreyThree: UIColor {
    return UIColor(red: 239.0 / 255.0, green: 241.0 / 255.0, blue: 244.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var steelTwo80: UIColor {
    return UIColor(red: 120.0 / 255.0, green: 129.0 / 255.0, blue: 154.0 / 255.0, alpha: 0.8)
  }

  @nonobjc class var apricot: UIColor {
    return UIColor(red: 1.0, green: 199.0 / 255.0, blue: 106.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var orangeish: UIColor {
    return UIColor(red: 253.0 / 255.0, green: 134.0 / 255.0, blue: 50.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var almostBlack: UIColor {
    return UIColor(red: 8.0 / 255.0, green: 10.0 / 255.0, blue: 15.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var cloudyBlue: UIColor {
    return UIColor(red: 188.0 / 255.0, green: 196.0 / 255.0, blue: 220.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var slate: UIColor {
    return UIColor(red: 80.0 / 255.0, green: 90.0 / 255.0, blue: 118.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var paleGreyFour: UIColor {
    return UIColor(red: 222.0 / 255.0, green: 224.0 / 255.0, blue: 230.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var paleGreyFive: UIColor {
    return UIColor(red: 240.0 / 255.0, green: 240.0 / 255.0, blue: 242.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var darkFive: UIColor {
    return UIColor(red: 20.0 / 255.0, green: 23.0 / 255.0, blue: 31.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var dark: UIColor {
    return UIColor(red: 23.0 / 255.0, green: 29.0 / 255.0, blue: 42.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var white30: UIColor {
        return UIColor(white: 1.0, alpha: 0.3)
  }
}
